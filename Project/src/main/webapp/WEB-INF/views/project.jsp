...
<body>
   ...
   <c:choose>
          ...
          <table>
            ...
            <c:forEach var="project" items="${PROJECT_LIST}"
               ...
               <tr>
                  ...
                  <c:if test="${EMPLOYEE.position.isAdmin()}">
                	 <td>
               	      <input type="button" value="Change" onClick="sendPost('/project/projectEdit.do','${project.id}')"/>
                  	</td>
                  </c:if>
               </tr>
            </c:forEach>
                  ...
                  <td>
                     <c:choose>
                       <c:when test="${not member.role.isDeveloper()}">
                	      <input type="button" value="Change" onClick="sendPost('/project/projectEdit.do','${member.project.id}')"/>
                       </c:when>
                      <c:otherwise>
      ...
      <%-- if user send id of project VIEW ONLY PROJECT WITH ID --%>
      <c:when test="${not empty PROJECT}">
         <P>Project name:${PROJECT.name}</P>
          <%-- This option will available for admin if he added himself in member list of this project --%>
         <c:if test="${not MEMBER.role.isDeveloper() || EMPLOYEE.position.isAdmin()}">
            <div>
               ...
               <table>
                  ...
                  <c:forEach var="task" items="${PROJECT_TASKS}"
                     ...
                     <tr>
                        ...
                        <td>
                        <c:choose>
                          <c:when test="${not MEMBER.role.isDeveloper() || EMPLOYEE.position.isAdmin()}">
                             <input type="button" onClick="sendPost('/project/taskEdit.do','${task.id}')" value="Change">
                          </c:when>
                          <c:otherwise>
                             No available
                          </c:otherwise>
